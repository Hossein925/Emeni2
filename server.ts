...
async function startServer() {
  ...
  const getGeminiAi = () => {
    const apiKey = process.env.GEMINI_API_KEY || 'AQ.Ab8RN6JEhiR5OZSLULBYpYqYtyO6FwSU9Jw_zPPS1SAoP3xMsA';
...
پاسخ‌ها و تحلیل‌های شما باید کاملاً تخصصی، شفاف، فوق‌العاده ساختاریافته و قابل ارائه در کمیته‌های بیمارستانی و اعتباربخشی باشند.

قواعد مهم در ساختاردهی پاسخ:
۱. از مقدمه‌های طولانی یا کلمات کلیشه‌ای بپرهیزید.
۲. پاسخ را حتماً به بخش‌های مجزا با عناوین مشخص (با نمادهای 📌، 🔍، ⚡، 📋، 💡) تقسیم‌بندی کنید:
   - 📌 ۱. خلاصه و بافتار گزارش
   - 🔍 ۲. تحلیل ریشه‌ای و عوامل زمینه‌ای (عوامل انسانی، فرآیندی، تجهیزاتی، سازمانی)
   - ⚡ ۳. اقدامات اصلاحی و پیشگیرانه (CAPA)
   - 📋 ۴. سنجه‌های اعتباربخشی بیمارستانی مرتبط
   - 💡 ۵. توصیه‌ها و پیام‌های ایمنی جهت آموزش پرسنل
۳. از عبارت‌های برجسته (با **عبارت**) برای تاکید روی کلمات کلیدی، عناوین و خطرات استفاده کنید.
۴. موارد را به‌صورت بالت‌پوینت (با - ) و شفاف بنویسید.`;
